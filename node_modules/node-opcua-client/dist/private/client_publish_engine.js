...
class ClientSidePublishEngine {
    ...
    suspend(suspendedState) {
        node_opcua_assert_1.assert(this.isSuspended !== !!suspendedState, "invalid state");
    ...
    registerSubscription(subscription) {
        ...
        node_opcua_assert_1.assert(_.isFinite(subscription.subscriptionId));
        node_opcua_assert_1.assert(!this.subscriptionMap.hasOwnProperty(subscription.subscriptionId)); // already registered ?
        node_opcua_assert_1.assert(_.isFunction(subscription.onNotificationMessage));
        node_opcua_assert_1.assert(_.isFinite(subscription.timeoutHint));
    ...
    unregisterSubscription(subscriptionId) {
        ...
        node_opcua_assert_1.assert(_.isFinite(subscriptionId) && subscriptionId > 0);
        this.activeSubscriptionCount -= 1;
    ...
    _send_publish_request() {
        ...
        node_opcua_assert_1.assert(!this.isSuspended, "should not be suspended");
        ...
        session.publish(publishRequest, (err, response) => {
            this.nbPendingPublishRequests -= 1;
            if (err) {
                debugLog(chalk_1.default.cyan("ClientSidePublishEngine.prototype._send_publish_request callback : "), chalk_1.default.yellow(err.message));
                debugLog("'" + err.message + "'");
                if (err.message.match("not connected")) {
                    debugLog(chalk_1.default.bgWhite.red(" WARNING :  CLIENT IS NOT CONNECTED :" +
                ...
                if (err.message.match(/BadNoSubscription/) && this.activeSubscriptionCount >= 1) {
                    ...
                    debugLog(chalk_1.default.bgWhite.red(" WARNING :   SERVER TELLS THAT IT HAS NO SUBSCRIPTION , " +
                        "BUT CLIENT DISAGREE"));
                    debugLog("this.activeSubscriptionCount =", this.activeSubscriptionCount);
                    active = false;
                }
                ...
                if (err.message.match(/BadTooManyPublishRequests/)) {
                    ...
                    active = false;
                    debugLog(chalk_1.default.bgWhite.red(" WARNING : SERVER TELLS THAT TOO MANY" +
                ...
                if (doDebug) {
                    debugLog(chalk_1.default.cyan("ClientSidePublishEngine.prototype._send_publish_request callback "));
                }
    ...
    _republish(subscription, subscriptionId, callback) {
        ...
        const sendRepublishFunc = (callback2) => {
            node_opcua_assert_1.assert(_.isFinite(subscription.lastSequenceNumber) &&
    ...
    __repairSubscription(subscription, subscriptionId, callback) {
        ...
        this._republish(subscription, subscriptionId, (err) => {
            ...
            if (err && err.message.match(/SubscriptionIdInvalid/)) {
                ...
                // the event handlers
                //
                debugLog(chalk_1.default.bgWhite.red("_republish failed " +
